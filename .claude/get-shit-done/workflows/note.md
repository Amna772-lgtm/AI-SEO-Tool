...
- **Global scope**: `D:/AI SEO Tool/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/AI SEO Tool/.claude/notes/`)
...
2. Glob `D:/AI SEO Tool/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/AI SEO Tool/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/AI SEO Tool/.claude/notes/` — works in any directory
